...
for strat in "${strategies[@]}"; do
    ...
    dbx-smith-spin "$strat" "$box_name" "$TEST_IMAGE" "dbs-$strat"
    ...
    if echo "$payload_content" | grep -qE "_dbx_smith_(precmd|prompt)\(\)"; then
        ...
        echo "--- Captured Payload Content ---"
        echo "$payload_content"
        echo "--------------------------------"
        exit 1
    fi

    # 6.5. Validation: Zsh Newuser Wizard Bypass
    echo "[*] Testing Zsh Wizard Bypass..."
    if ! (cd / && dbx-smith "$box_name" -- sh -c 'test -f ~/.zshrc' 2>/dev/null); then
        echo "[!] ERROR: ~/.zshrc missing! Zsh newuser wizard will trigger interactively."
        exit 1
    fi
    echo "[*] Validation Passed: Empty .zshrc successfully bootstrapped."
    ...
    # 7.5. Validation: Sudo & Filesystem Isolation
    ...
    echo "[*] Validation Passed: Sudo works without password."

    if [[ "$strat" == "airgapped" || "$strat" == "ghost-airgapped" || "$strat" == "isolated-net" || "$strat" == "ghost-isolated-net" ]]; then
        echo "[*] Testing tmpfs home isolation..."
        # If it's a ghost strategy with isolation, the host user directory should NOT exist in /home
        if [[ "$strat" == ghost* ]]; then
        ...
        echo "[*] Validation Passed: Home directory is isolated."
    fi
    ...
    # 7.6. Validation: Identity-based Isolation (Ghost only)
    if [[ "$strat" == ghost* ]]; then
        echo "[*] Testing Identity-based Isolation (UID mismatch)..."
        # Attempt to access host user directory should FAIL with Permission Denied
        if (cd / && dbx-smith "$box_name" -- ls "/run/host/home/$USER" >/dev/null 2>&1); then
             # If it's a standard ghost box (no tmpfs), it's mounted but should be PERMISSION DENIED
             # If it's isolated ghost (tmpfs), it's hidden entirely.
             # Either way, ghostuser should NOT be able to list it.
             echo "[!] ERROR: Ghost user can access host home directory! Isolation failure."
             exit 1
        fi
        echo "[*] Validation Passed: Host home is inaccessible to Ghost identity."
    fi
