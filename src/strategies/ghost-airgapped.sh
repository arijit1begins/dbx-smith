...
strategy_ghost_airgapped_get_flags() {
    ...
    local airgap_hook="if [ -f /etc/dbx_airgap_active ]; then for iface in \$(ls /sys/class/net/ 2>/dev/null); do if [ \"\$iface\" != \"lo\" ]; then ip link set \"\$iface\" down 2>/dev/null || true; fi; done; fi"
