...
print_usage() {
    ...
    echo -e "  ./test.sh --strategy ghost          # Test Ghost strategy across all discovered distros"
}
...
# 1. Install Suite Core Binaries once globally before slaves execute
log_info "Ensuring latest suite code is globally installed..."
make install >"${RUN_LOG_DIR}/master_make_install.log" 2>&1 || {
    log_error "Global make install failed! Check log: ${RUN_LOG_DIR}/master_make_install.log"
    exit 1
}
...
if [[ -d "tests/unit" ]]; then
    for unit_file in tests/unit/*.sh; do
        ...
        if bash "$unit_file" >"$unit_log" 2>&1; then
